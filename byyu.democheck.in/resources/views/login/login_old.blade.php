                                        ...
                                        <fieldset class="login_number_box mb-3">
                                            <div class="label_list">
                                                <div id="phone-container" class="input-group">
                                                    ...
                                                    <input type="hidden" id="appUrl" name="country_code" value="{{ env('APP_URL') }}">
                                                    <input type="hidden" id="NODE_APP_URL" name="country_code" value="{{ env('NODE_APP_URL') }}">
...
<script>
    ...
    document.querySelector("#login-form").addEventListener("submit", function(event) {
        ...
        if (iti.isValidNumber() && errorMsg.classList.contains("hide")) {
            ...
            const formData = {
                ...
                dialcode: iti.getSelectedCountryData().dialCode, // Get the actual dial code
                device_id: "" // Add actual device ID if available
            };
                ...
                success: function(response) {
                    ...
                    if (response.status == '1') {
                        ...
                        // alert(userId)
                            ...
                            window.location.href = appUrl + "loginotp/" + userId;

                    } else if (response.status == '2') {
                        window.location.href = appUrl + "register";
                    } else {
                        alert(response.message); // Show an alert with the response message if the status is not 1 or 2
                    }
                },
